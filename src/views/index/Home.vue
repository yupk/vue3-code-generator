<template>
	<div class="container">
		...
		<div class="center-board">
			...
			<el-scrollbar class="center-scrollbar" :class="device">
				<el-form :size="formConf.size" :label-position="formConf.labelPosition" :disabled="formConf.disabled"
					:label-width="formConf.labelWidth + 'px'" style="padding: 10px">
					<draggable class="drawing-board" component="el-row" :gutter="formConf.gutter" :list="drawingList"
		...
		<div>
			<json-drawer size="60%" v-model="jsonDrawerVisible" :json-str="JSON.stringify(formData)"
				@refresh="refreshJson" />
		</div>
...
<script>
	...
	export default  {
		...
		methods: {
			...
			updateCode() {
				this.AssembleFormData();
				this.generateConf = this.formConf;
				const codeStr = this.generateCode();
				this.$emit("updateCode", codeStr);
			},
			execCopy(data) {
