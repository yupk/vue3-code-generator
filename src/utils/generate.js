...
class Scripts {
    ...
    renderSetup(lines) {
        lines.push("setup(props, context) {");
        lines = this.renderFormData(lines);
        lines = this.renderUIData(lines);
        lines = this.renderApiScript(lines);
        lines = this.renderFormRules(lines);
        let rentrnData = this.returnData.data().join(",");
        lines.push(` return { ${rentrnData} }`)
        lines.push("},");
        return lines;
    }
...
const toHtml = function (ele, js) {
    ...
    let node = ["<", tagName, " ", attrFormat(ele.attrs, ele.props), " ", ">\n", childrenFormat(ele.childrens, js), slotFormat(ele.slots), "</", tagName, ">\n"]
    if (ele.formItem) {
...
const generate = function (settings) {
    ...
    return ["<template>", html_beautify(html), '</template>', "<script>", js_beautify(js.render()), "</script>"].join("\n")
